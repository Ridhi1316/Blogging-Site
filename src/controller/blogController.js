...
const createBlog = async function (req, res) {
  try {
    let data = req.body;
    if(data.title==0){
      return res.status(400).send({data:"Title is required!!",status:false});
   }
  if(data.body==0){
     return res.status(400).send({data:"Body is required!!",status:false});
    }
 if(data.tags==0){
     return res.status(400).send({data:"Tags is required!!",status:false});
    }
 if(data.category==0){
  return res.status(400).send({data:"Category is required!!",status:false});
  }
  if(data.isPublished==0){
    return res.status(400).send({data:"Published Status is required!!",status:false});
    }
